...
const CHARACTERS = [
  ...
  },
  {
    ...
    epithet: "Dream-Eater of the Floating World",
    element: "anemo",
    role: "On-Field Swirl Driver / Healer",
    weaponType: "Catalyst",
    set: "4pc Viridescent Venerer",
    setAlt: "4pc Gilded Dreams · 2pc VV + 2pc +80 EM (transitional)",
    weapon: "Sunny Morning Sleep-In",
    weaponAlt: "Sacrificial Fragments (4★ — Skill reset) · A Thousand Floating Dreams · Favonius Codex (4★ ER) · Mappa Mare (F2P craftable)",
    talents: "Skill › Burst ›› Normal (1/9/9)",
    stats: [
      { id: "sands", label: "Sands main stat", target: "EM (ER if Burst uptime slips)" },
      { id: "goblet", label: "Goblet main stat", target: "EM" },
      { id: "circlet", label: "Circlet main stat", target: "EM" },
      { id: "em", label: "Total Elemental Mastery", target: "≥ 900 sheet (1000+ in Dreamdrifter)" },
      { id: "er", label: "Energy Recharge", target: "150 – 180 %" },
    ],
    note: "Pure EM, zero CRIT. Her Swirl damage, the party-wide Swirl DMG bonus, the elemental DMG bonus she hands out (0.04 % per EM point to Pyro/Hydro/Cryo/Electro) and her healing ALL scale off Elemental Mastery — there is no stat on her that CRIT competes with. Her in-combat EM runs far above the character sheet: the weapon stacks up to +248 and A4 adds +100 whenever a teammate lands one of the four elements during Dreamdrifter.",
    altBuilds: [
      {
        id: "stellar-swirl",
        label: "Stellar-Swirl (leak)",
        role: "Stellar-Swirl Anemo Carry — UNRELEASED",
        set: "4pc Viridescent Venerer",
        setAlt: "Pending — Snezhnaya is expected to ship a dedicated Stellar set",
        weapon: "Sunny Morning Sleep-In",
        weaponAlt: "A Thousand Floating Dreams · Sacrificial Fragments (4★)",
        talents: "Skill › Burst ›› Normal",
        stats: [
          { id: "ss-sands", label: "Sands main stat", target: "EM" },
          { id: "ss-goblet", label: "Goblet main stat", target: "EM" },
          { id: "ss-circlet", label: "Circlet main stat", target: "CRIT Rate / DMG ⚠ speculative" },
          { id: "ss-em", label: "Total Elemental Mastery", target: "≥ 800 — still the primary scaler" },
          { id: "ss-cr", label: "CRIT Rate", target: "50 – 60 % ⚠ only if the trigger can CRIT" },
    ...
    deep: {
      lore: "An Inazuman youkai who tends other people's dreams — and quietly makes a meal of the nightmares she draws out of them. In the Dreamdrifter state she stops walking altogether, drifting a hand's breadth above the ground while the wind carries off whatever elements her companions leave hanging in the air.",
      kit: [
        "Skill — Aisa Utamakura Pilgrimage: enters the floating Dreamdrifter state, dealing AoE Anemo DMG at intervals and granting the whole party a Swirl DMG bonus scaled on her EM. This is the engine the entire kit runs on.",
        "Dreamdrifter's second channel: nearby party members also gain Pyro/Hydro/Cryo/Electro DMG Bonus equal to 0.04 % per point of Mizuki's EM — a separate buff stacked on top of the Swirl bonus.",
        "Burst — Anraku Secret Spring Therapy: AoE Anemo DMG plus the team healing, which also scales off her Elemental Mastery.",
        "A1 (Bright Moon's Restless Voice): every Swirl she triggers extends Dreamdrifter by 2.5 s, up to twice per 0.3 s — with decent Swirl uptime the state largely sustains itself.",
        "A4 (Thoughts by Day Bring Dreams by Night): whenever a party member hits an opponent with Pyro/Hydro/Cryo/Electro during Dreamdrifter, Mizuki gains +100 EM for 4 s.",
      ],
      tips: [
        "Talents 1/9/9 — her Normal Attack contributes nothing. Skill carries the Swirl scaling, Burst carries the healing.",
        "Feed her one or two off-field applicators of a single element. Swirl only converts what is already in the air, and a crowded elemental board splits her output instead of compounding it.",
        "Sunny Morning Sleep-In stacks +120 EM on Swirl, +96 on Skill hit and +32 on Burst hit — up to +248 EM from the weapon alone, which is why it beats generic EM catalysts by a wide margin.",
      ],
      synergies: [
    ...
    reactions: [
      {
        ...
        trigger: "Superconduct + Stellar enabler",
        elements: ["cryo", "electro"],
        short: "Superconduct variant expected to debut in 6.7 alongside Sandrone.",
        multiplier: "Reported CRIT-capable ⚠ unconfirmed",
        details: [
          "Described as a Cryo + Electro variant of Superconduct gated behind specific characters — the same design shape as Lunar reactions being gated behind a Moonsign character.",
          "Expected in Version 6.7, the same patch as Sandrone, whose kit is built around Stellar-Conduct as its damage vehicle.",
      ...
      },
      {
        id: "stellar-swirl",
        name: "Stellar-Swirl",
        trigger: "Anemo on Cryo + Stellar enabler",
        elements: ["anemo", "cryo"],
        short: "Anemo-into-Cryo variant that grows a Polestar Wind Cluster, then detonates it.",
        multiplier: "Growing AoE Cryo detonation; reported CRIT-capable ⚠",
        details: [
          "Anemo contact with a Cryo-affected target is said to spawn a Polestar Wind Cluster: Cryo — a field that grows each time Stellar-Swirl re-triggers nearby, then explodes for large AoE Cryo DMG over a wider area.",
          "Both the initial Anemo hit and the detonation are reported to CRIT. If that survives to release it is a genuine build shift: it would be the first time a Swirl-line reaction rewards CRIT stats on the Anemo trigger, breaking the pure-EM orthodoxy that every Anemo Swirl carry has been built on.",
          "Requires an Anemo carry hitting into Cryo with a dedicated Stellar-Swirl enabler on the team — and that enabler is not a released character.",
...
function getRouteFromHash() {
  ...
  const key = raw.replace(/^#\/?/, "").replace(/^genshin\/?/, "").toLowerCase();
...
export default function GenshinApp() {
  ...
  const goToCharacter = (id) => {
    ...
    if (route.page !== ROUTES.characters) {
      ...
      window.location.hash = "#/genshin/characters";
      return;
    }
...
function NavBar({ route }) {
  ...
  return (
    <nav className="nav" aria-label="Primary">
      <a className="nav-brand" href="#/genshin/characters">
    ...
    return (
      <>
        ...
        <div className="region-stub">
          <a className="region-back" href="#/genshin/regions">← Back to regions</a>
        </div>
      </>
    );
  }
              ...
              {char.deep.synergies && char.deep.synergies.length > 0 && (
                <div className="syn-list">
                  ...
                  {char.deep.synergies.map((s) => {
                    const target = roster.find((c) => c.id === s.id);
                    if (!target) return null;
                    const tTheme = themeMap[target.element];
                    return (
                      <button
                        key={s.id}
                        type="button"
...
export { CharacterCard, StatRow, styles as almanacStyles };
